// src/components/Phase2UrlMode.tsx

import React, { useState } from "react";
import { toast } from "sonner";
import { cn } from "@/lib/utils";
import { submitUrl } from "@/api"; // ✅ import axios API function
    ...
    try {
      ...
      const timer = setInterval(() => {
        ...
        if (currentProgress >= 100) {
          ...
          setTimeout(() => {
            setIsLoading(false);
            toast.success(`Successfully submitted URL: ${url}`);
            onComplete();
          }, 300);
        }
      }, 300);
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground">
          Enter the URL of your web application to automatically scan and
          analyze it for test case generation.
        </p>
      </div>
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-2">
                {capturedScreenshots.map((screenshot, index) => (
                  <div
                    key={index}
                    className="p-2 bg-secondary rounded flex items-center space-x-2 animate-fade-in"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="16"
