...
import { fetchGeneratedCode } from "@/api"; // ✅ IMPORT backend API

interface Phase5Props {
  onComplete: () => void;
  testCases: TestCase[]; // ✅ pass real test cases instead of MOCK
}

export default function Phase5CodeGeneration({ onComplete, testCases }: Phase5Props) {
  ...
  const generateCode = async () => {
    ...
    try {
      const code = await fetchGeneratedCode(selectedTestCase.id); // ✅ call backend
  ...
  const saveCode = () => {
    ...
    toast.success("Test code saved successfully (mock)");
  };
