...
from datetime import datetime
from pathlib import Path
from playwright.sync_api import sync_playwright  # ✅ Sync version for CLI/test usage
...
def launch_browser_sync():
    try:
        with sync_playwright() as p:
            ...
            page.goto("https://example.com")  # ✅ You can update to your desired test URL
...
def run_latest_generated_story_test():
    try:
        # ✅ Step 1: Launch browser (blocking)
        launch_browser_sync()

        # ✅ Step 2: Find the latest test file
        test_files = sorted(
            [f for f in tests_dir.glob("test_*.py") if f.is_file()],
            key=lambda x: x.stat().st_mtime,
            reverse=True
        ...
        # ✅ Step 3: Prepare log and meta files
        logs_dir.mkdir(parents=True, exist_ok=True)
        meta_dir.mkdir(parents=True, exist_ok=True)
        log_file = logs_dir / f"test_output_{latest_test_path.stem}.log"
        meta_file = meta_dir / f"execution_metadata_{latest_test_path.stem}.json"
